// Dashboard panel — the default landing view. A mix of personal + team signals.
// Layout: a hero stat row + 3-column grid below (left = tasks/quick add,
// middle = mini board snapshot, right = activity feed + upcoming).

function Dashboard({ onJumpToKanban, onOpenTask, onNewTask }) {
  ...
  const overdue = allTasks.filter(t => t.due === 'Today');
    ...
    <div data-screen-label="Dashboard" style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginTop: 18 }}>
        <StatCard
          ...
          value={today.length + 4}
          delta={{ dir: 'up', text: '2 added today' }}
          accent="accent"
          icon="calendar"
        />
        <StatCard
          label="In progress"
          value={inProgress}
          delta={{ dir: 'flat', text: 'across 3 boards' }}
          accent="info"
          icon="zap"
        />
        <StatCard
          ...
          value={review}
          delta={{ dir: 'up', text: '+1 since yesterday' }}
          accent="warning"
          icon="checkcir"
        />
        <StatCard
          ...
          value={doneCount + 38}
          delta={{ dir: 'up', text: `${completion}% of board done` }}
          accent="success"
          icon="trending"
        />
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <input
          ...
          style={{
            ...
            fontFamily: 'inherit', fontSize: 13.5, color: 'var(--fg)',
...
// ---- today tasks -------------------------------------------------------
function TodayTasks({ tasks, onOpenTask }) {
  return (
...
function TaskRow({ task, onOpen, divider }) {
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2, fontSize: 11, color: 'var(--fg-subtle)' }}>
          ...
          <span>·</span>
          <StatusBadge status={task.col} label={status?.name || 'Backlog'} />
        </div>
      </div>
      <AvatarStack people={task.assignees.map(k => ({ initials: PEOPLE[k].initials }))} size={20} max={2} />
    </div>
  );
}
...
// ---- board snapshot ----------------------------------------------------
function BoardSnapshot({ allTasks, onJumpToKanban }) {
  ...
  return (
    <Panel
      ...
      action={<Button variant="ghost" size="sm" icon="arrow_rt" onClick={onJumpToKanban}>Open board</Button>}
    >
      {/* Distribution bar */}
      <div style={{ display: 'flex', height: 8, borderRadius: 999, overflow: 'hidden', margin: '4px 0 14px' }}>
        {counts.map(c => {
          const color = c.id === 'backlog' ? 'var(--neutral-300)' : c.id === 'in_progress' ? 'var(--info)' : c.id === 'in_review' ? 'var(--warning)' : 'var(--success)';
          return <span key={c.id} style={{ width: `${(c.n / total) * 100}%`, background: color }} title={`${c.name}: ${c.n}`} />;
        })}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {counts.map(c => {
          const tasks = allTasks.filter(t => t.col === c.id).slice(0, 3);
            ...
            <div key={c.id} style={{
              ...
              {tasks.map(t => (
                <div key={t.id} style={{
                  ...
                  fontSize: 11, lineHeight: 1.3, letterSpacing: '-0.005em',
              ...
              {tasks.length === 0 && (
                <div style={{ fontSize: 10.5, color: 'var(--fg-faint)', padding: '8px 4px', textAlign: 'center' }}>—</div>
              )}
            </div>
          );
        })}
...
// ---- my tasks --------------------------------------------------------
function MyTasksBlock({ myTasks, onOpenTask }) {
  return (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 4 }}>
        {upcoming.map(t => (
          <div key={t.id} onClick={() => onOpenTask(t.id)} className="zt-row" style={{
            ...
            <DueChip due={t.due} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12.5, fontWeight: 500, letterSpacing: '-0.005em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {t.title}
              </div>
            ...
            <div key={i} style={{ display: 'flex', gap: 10, minWidth: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12.5, lineHeight: 1.45, color: 'var(--fg)', letterSpacing: '-0.005em' }}>
                  ...
                  {a.to && <span style={{ color: 'var(--fg-muted)' }}> → <span style={{ color: 'var(--fg)', fontWeight: 500 }}>{a.to}</span></span>}
                </div>
                {a.text && (
                  <div style={{
                    marginTop: 4,
...
function Empty({ label }) {
  return (
    <div style={{ padding: '14px 6px', textAlign: 'center', color: 'var(--fg-subtle)', fontSize: 12 }}>{label}</div>
  );
}
